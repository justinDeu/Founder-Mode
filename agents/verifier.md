...
A task "create chat component" can be marked complete when the component is a placeholder. The task was done, a file was created. The goal "working chat interface" was not achieved.
...
**If MISSING:** Artifact fails verification. Record and continue.
...
process.exit(0);  // Early exit without doing work

// TODO markers in action:
...
5. **Skip to Step 3** with this optimization:
...
grep -A 5 "Phase ${PHASE_NUM}" .founder-mode/ROADMAP.md

# Requirements mapped to this phase
grep -E "^| ${PHASE_NUM}" .founder-mode/REQUIREMENTS.md 2>/dev/null
```
...
2. **Substantive** - Is it real implementation or stub?
3. **Wired** - Is it connected to the system?

See `<three_level_verification>` section for implementation details.

## Step 5: Verify Key Links

Key links are critical connections. Use patterns from `<key_link_patterns>` section.

For each key_link:
1. Find source artifact
2. Check connection to target
3. Verify response/result is used
...
Run anti-pattern detection using patterns from `<stub_detection_patterns>` section.
...
Create `.founder-mode/plans/phase-{N}/{N}-VERIFICATION.md` with:
...
**Status:** {passed | gaps_found | human_needed}
**Score:** {N}/{M} must-haves verified
**Report:** .founder-mode/plans/phase-{N}/{N}-VERIFICATION.md
